...
void INVITARJUGADOR(int sockets, const char *nombreUsuarioInvitador)
{
	// Recibir la invitaciￃﾳn del cliente
	char Usuario[1024];
	recv(sockets, Usuario, sizeof(Usuario), 0);
	
	// Realizar acciones adicionales con la invitaciￃﾳn recibida
	printf("Invitacicion recibida de %s: %s\n", nombreUsuarioInvitador, Usuario);
	
	// Enviar respuesta al cliente invitador
	char Respuesta[100];
	printf("�Aceptas la invitaciacion? (Si/No): ");
	scanf("%s", Respuesta);
	// Enviar la respuesta al cliente que enviￃﾳ la invitaciￃﾳn
	send(sockets, Respuesta, strlen(Respuesta), 0);
	
	// Puedes realizar mￃﾡs acciones dependiendo de la respuesta del usuario invitado
}
...
void *AtenderCliente (void *socket)
{
	...
	while (terminar == 0)
	{
		...
		case 7: //Consulta para pedir la lista de usuarios conectados
		{
			char nombreUsuarioInvitador[100];
			int sockets;
			INVITARJUGADOR(sockets, nombreUsuarioInvitador);
		}
		break;
		}
	}
